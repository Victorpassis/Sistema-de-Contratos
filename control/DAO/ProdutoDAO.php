...
class ProdutoDAO {
   function cadastrar(Produto $produto) {
      try {
         ...
         $cadastro = $con->prepare("INSERT INTO produtos(nomeProduto,descricao,valor) VALUES(?,?,?)");
   ...
   function editar(Produto $produto, $idProduto) {
      try {
         ...
         $cadastro = $con->prepare("UPDATE produtos SET nomeProduto = ?,valor = ?,descricao = ? WHERE idProduto = ".$idProduto);

         $cadastro->bindValue(1,$produto->getNome());
         $cadastro->bindValue(2,$produto->getValor());
         $cadastro->bindValue(3,$produto->getDescricao());
