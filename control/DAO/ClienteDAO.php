...
class ClienteDAO {
   function cadastrar(Cliente $cliente) {
      try {
         ...
         if($cadastro->rowCount() > 0) {
            ...
            $cadastro = $con->prepare("INSERT INTO clientes(nomeCliente,dados,cidade,cep,estado,endereco,numero,bairro) VALUES(?,?,?,?,?,?,?,?)");
   ...
   function editar(Cliente $cliente, $idCliente) {
      try {
         ...
         $cadastro = $con->prepare("UPDATE clientes SET nomeCliente = ?,dados = ?,cidade = ?,cep = ?,estado = ?,endereco = ?,numero = ?,bairro = ? WHERE idCliente = ".$idCliente);
